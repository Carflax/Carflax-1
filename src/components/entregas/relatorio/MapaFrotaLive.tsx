...
import { apiFrotaPosicoes, apiEntregasMapa, type FrotaPosicao, type EntregaMapa } from "@/lib/api";
import carflaxLogo from "@/assets/Carflax.png";

// Mapa 2D via Leaflet: os tiles são <img> no DOM (SEM WebGL), então renderiza
// em qualquer máquina. Puxamos posições + trilha real a cada 10s e o carro
...
export function MapaFrotaLive() {
  ...
  const clientesRef = useRef<globalThis.Map<string, L.Marker>>(new globalThis.Map());
  const rafRef = useRef<globalThis.Map<string, number>>(new globalThis.Map());
  const lastTsRef = useRef<globalThis.Map<string, string>>(new globalThis.Map());
  ...
  const glide = useCallback((placa: string, marker: L.Marker, pontos: L.LatLng[]) => {
    ...
    const step = (now: number) => {
      ...
      const a = pontos[i];
      const b = pontos[i + 1];
      marker.setLatLng([a.lat + (b.lat - a.lat) * f, a.lng + (b.lng - a.lng) * f]);
      ...
      for (const p of posicoes) {
        ...
        if (existente) {
          ...
          if (novos.length > 0) {
            ...
            const caminho = [existente.getLatLng(), ...novos.map((pt) => L.latLng(pt.lat, pt.lng))];
            glide(p.placa, existente, caminho);
            lastTsRef.current.set(p.placa, String(novos[novos.length - 1].dataHora));
          } else if (trilha.length === 0) {
            // Sem trilha disponível: cai no deslocamento reto até a última posição.
            const cur = existente.getLatLng();
            if (Math.abs(cur.lat - p.latitude) > 1e-7 || Math.abs(cur.lng - p.longitude) > 1e-7) {
              glide(p.placa, existente, [cur, ll]);
            }
          }
        } else {
          const m = L.marker(ll, { icon: iconeMotorista(p) }).addTo(map).bindPopup(popupHtml(p));
          ...
          lastTsRef.current.set(
            p.placa,
            trilha.length ? String(trilha[trilha.length - 1].dataHora) : String(p.dataHora || ""),
          );
        }
      }
  ...
  useEffect(() => {
    ...
    const map = L.map(boxRef.current, { zoomControl: false, attributionControl: false }).setView(CENTRO_PADRAO, 12);
    ...
    L.marker([DEPOSITO.lat, DEPOSITO.lng], {
      ...
      .addTo(map)
      .bindPopup(`<div style="font:400 12px/1.4 ui-sans-serif,system-ui"><b>Carflax</b><br>Av. Américo Bruno, 75 · Jundiaí/SP</div>`);
    ...
    try {
      ...
      for (const c of clientes) {
        ...
        if (existente) {
          ...
          const m = L.marker([c.lat, c.lng], { icon: iconeCliente(c.status) }).addTo(map).bindPopup(popupClienteHtml(c));
      ...
      {loading && total === 0 && (
        <div className="absolute inset-0 z-[400] flex items-center justify-center bg-background/40 backdrop-blur-sm">
          <div className="flex items-center gap-2 rounded-xl bg-card border border-border px-4 py-3 shadow-lg">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-primary">Localizando frota…</span>
          </div>
        </div>
      )}
    </div>
